...
export default function WhatADrag(Map) {
    ...
    // console.debug(CheapestPaths);

    let Seats = 0;
    let SeatsPath = {};

    for (const cheapPath of CheapestPaths) {
        if (cheapPath.cost !== Cheapest) {
            continue;
        }

        let path = cheapPath.steps;
