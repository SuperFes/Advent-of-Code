...
fun main(args: Array<String>) {
    ...
    for (sensor in sensors.sortedBy { it.ManDist }) {
        var doX = sensor.Left
        var doY = sensor.y

        while (!solved && doX <= sensor.x && doY <= sensor.Bottom) {
            if (doY in coordY..coordMaxY && doX in coordX .. coordMaxX) {
                for (sensorDeux in sensors) {
                    // This one is us
                    if (sensorDeux.x == sensor.x && sensorDeux.y == sensor.y) {
                        continue
                    }

                    if (MrManhattan(doX - 1, doY + 1, sensorDeux.x, sensorDeux.y) <= sensorDeux.ManDist) {
                        bumped = true
        ...
        while (!solved && doX <= sensor.Left && doY >= sensor.y) {
            if (doY in coordY..coordMaxY && doX in coordX .. coordMaxX) {
                for (sensorDeux in sensors) {
                    // This one is us
                    if (sensorDeux.x == sensor.x && sensorDeux.y == sensor.y) {
        ...
        while (!solved && doX >= sensor.x && doY <= sensor.Top) {
            if (doY in coordY..coordMaxY && doX in coordX .. coordMaxX) {
                for (sensorDeux in sensors) {
                    // This one is us
                    if (sensorDeux.x == sensor.x && sensorDeux.y == sensor.y) {
